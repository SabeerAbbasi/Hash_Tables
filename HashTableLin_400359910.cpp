...
void HashTableLin::insert(int n) {
    if (!isIn(n)) { // if n is not already in the table
        ...
        while (table[i] != -1) { // linear probing until an empty slot is found
            i = (i + 1) % size;
            count++;
...
bool HashTableLin::isIn(int n) // Define the function and its parameter
{
...
int HashTableLin::insertCount(int n) {
    ...
    while (!inserted && probe < size) { //continues until the key is inserted or the table is full
        ...
        i = (i + 1) % size; // Calculate the next hash index using linear probing
    }
...
std::vector<double> HashTableLin::simProbeSuccess()
{
    ...
    for (double load_factor = 0.1; load_factor < 0.99; load_factor += 0.1) {
        ...
        for (int trial = 0; trial < trial_count; trial++) {
            HashTableLin hashtable(max_size, load_factor);
